    ...
    <div class="table-responsive">
        <table class="table table-bordered ">
            ...
            <tr>
                ...
                <td>
                    @if ($bank_transfer_payment->status == 'Approved')
                        <span
                            class="bg-success p-1 px-3 rounded text-white">{{ ucfirst($bank_transfer_payment->status) }}</span>
                    @elseif($bank_transfer_payment->status == 'Pending')
                        <span
                            class="bg-warning p-1 px-3 rounded text-white">{{ ucfirst($bank_transfer_payment->status) }}</span>
                    @else
                        <span
                            class="bg-danger p-1 px-3 rounded text-white">{{ ucfirst($bank_transfer_payment->status) }}</span>
                    @endif
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <p><span
                            class="text-primary">{{ __('Workspace: ') }}</span>{{ $requests->workspace_counter_input }}
                    </p>
                    ...
                    <div class="">
                        ...
                        @foreach ($modules as $module)
                            @if ($module)
                                <a href="{{ route('software.details', Module_Alias_Name($module)) }}" target="_new"
                                    class="btn btn-sm btn-warning me-2">{{ Module_Alias_Name($module) }}</a>
                            @endif
                        @endforeach
                    </div>
                </td>
                ...
                <td>
                    @if (!empty($bank_transfer_payment->attachment) && check_file($bank_transfer_payment->attachment))
                        ...
                        <div class="action-btn bg-secondary ms-2">
                            <a class="mx-3 btn btn-sm align-items-center"
                                href="{{ get_file($bank_transfer_payment->attachment) }}" target="_blank">
