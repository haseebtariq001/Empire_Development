...
@section('page-action')
    <div>
        ...
        @isset($agencies)
            @can('agency create')
                <a class="btn btn-sm btn-primary" data-title="{{ __('Create New ' . $singular_name) }}"
                    href="{{ route('agency.add') }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Create') }}">
                ...
                <div class="card">
                    <div class="card-header border-0 pb-0">
                        <div class="d-flex align-items-center">
                            <span
                                @class([
                                    'badge p-2 px-3 rounded text-black',
                                    'bg-primary' => !isset($agencies),
                                    'bg-success' => isset($agencies) && $user->agency->status == 'Approved',
                                    'bg-warning' => isset($agencies) && $user->agency->status == 'pending',
                                    'bg-danger' => isset($agencies) && $user->agency->status == 'Rejected',
                                ])>{{ isset($agencies) ? $user->agency->status : $user->type }}</span>
                        </div>
                            ...
                            @can('user manage')
                                <div class="btn-group card-option">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end" data-popper-placement="bottom-end">
                                        @can('user edit')
                                            @isset($agencies)
                                                <a href="{{ route('agency.show', $user->agency->id) }}" class="dropdown-item"
                                                ...
                                                @if ($user->agency->status == 'pending')
                                                    <a href="#!" data-url="{{ route('users.reset', \Crypt::encrypt($user->id)) }}"
                                                        ...
                                                        data-title="{{ __('Set Password For the Agency') }}"
                                            ...
                                            @else
                                                ...
                                                <a data-url="{{ route('users.reset', \Crypt::encrypt($user->id)) }}" class="dropdown-item"
                                                    data-ajax-popup="true" data-title="{{ __('Reset Password') }}"
                                                    data-toggle="tooltip" data-original-title="{{ __('Reset Password') }}">
                            ...
                            <div class="mt-4">
                                <div class="row justify-content-between align-items-center">
                                    <div class="col-6 text-center">
                                        <span
                                            class="d-block font-bold mb-0">{{ !empty($user->plan) ? (!empty($user->plan->name) ? $user->plan->name : 'Basice Plan') : 'Plan Not Activated' }}</span>
                                    </div>
            ...
            @isset($agencies)
                @can('agency create')
                    <div class="col-md-3 All">
                        <a class="btn-addnew-project " style="padding: 90px 10px;"
                            data-title="{{ __('Create New ' . $singular_name) }}" href="{{ route('agency.add') }}">
                ...
                @can('user create')
                    <div class="col-md-3 All">
                        <a href="#" class="btn-addnew-project " style="padding: 90px 10px;" data-ajax-popup="true"
                            data-size="md" data-title="{{ __('Create New ' . $singular_name) }}"
