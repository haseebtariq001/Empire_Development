    ...
    function getSideMenu()
    {
        if (\Auth::check()) {
            if (\Auth::user()->type == "super admin") {
                ...
                return $menus = Sidebar::with('childs')->where('parent_id', '=', 0)->whereIn('type', ["agency"])->where('is_visible', 1)->orderBy('sort_order')->get();
            }elseif (\Auth::user()->type == "agent") {
                return $menus = Sidebar::with('childs')->where('parent_id', '=', 0)->whereIn('type', ["agent"])->where('is_visible', 1)->orderBy('sort_order')->get();
            }elseif (\Auth::user()->type == "empire agent") {
                return $menus = Sidebar::with('childs')->where('parent_id', '=', 0)->whereIn('type', ["agent", "empire agent"])->where('is_visible', 1)->orderBy('sort_order')->get();
            } else {
                return Cache::store('file')->rememberForever('cached_menu_auth' . \Auth::user()->id, function () {
                    $menus = Sidebar::select('sidebar.id', 'sidebar.parent_id', 'sidebar.module', 'sidebar.type', 'sidebar.is_visible', 'sidebar.route', 'sidebar.dependency', 'sidebar.disable_module', 'sidebar.permissions', 'sidebar.icon', 'sidebar.title')
                        ...
                        ->whereIn('sidebar.type', ['common', 'company'])
...
if (!function_exists('company_setting')) {
    function company_setting($key, $user_id = null, $workspace = null)
    {
        ...
        if (!in_array($user->type, ['company', 'super admin', 'agency'])) {
...
if (!function_exists('get_file')) {
    function get_file($path)
    {
        if (admin_setting('storage_setting') == 's3') {
            ...
            return asset('assets/images/image-placeholder.jpg');
        }
    }
}
...
if (!function_exists('save_image_in_storage_with_timestamp')) {
    function save_image_in_storage_with_timestamp($folder_name, $file)
    {
        $randomNumber = rand(10000, 99999);
        $filename = time() . '_' . $randomNumber . '.' . $file->getClientOriginalExtension();
        $file->storeAs("uploads/{$folder_name}", $filename);
        return $filename;
    }
}
        ...
        try {
            if (!empty(admin_setting('storage_setting'))) {
                if (admin_setting('storage_setting') == 'wasabi') {
                    ...
                    $mimes =  !empty(admin_setting('local_storage_validation')) ? admin_setting('local_storage_validation') : 'jpeg,jpg,png,svg,zip,txt,gif,docx,pdf';
                }
...
if (!function_exists('currency_format_without_symbol')) {

    function currency_format_without_symbol($price)
    {
         return number_format($price, company_setting('currency_format'));
    }
}
    ...
    function delete_file($path)
    {
        if (check_file($path)) {
            if (admin_setting('storage_setting') == 'local') {
                return File::delete('storage/' . $path);
            } else {
                if (admin_setting('storage_setting') == 's3') {
                    config(
                        [
...
if (!function_exists('calculate_percentage')) {
    function calculate_percentage($amount, $percent)
    {
...
if (!function_exists('format_currency')) {
    function format_currency($amount, $currency_code = 'AED', $decimal_places = 2)
    {
        $amount = (float) $amount;
        $amount = round($amount, $decimal_places); // Round the amount to the specified decimal places
        $formatted_currency = number_format($amount, $decimal_places, '.', ',');
        $formatted_currency = $currency_code . ' ' . $formatted_currency;
...
if (!function_exists('splitStringByCharacter')) {
    function splitStringByCharacter($string) {
        $splitPosition = strpos($string, ' - ');
        
        if ($splitPosition !== false) {
            // Found ' - ' delimiter
            $splitPosition += strlen(' - '); // Move past the delimiter
        } else {
            // If ' - ' delimiter is not found, try other delimiters
            $delimiters = [' ', ',', '.'];
            foreach ($delimiters as $delimiter) {
                $splitPosition = strrpos($string, $delimiter);
                if ($splitPosition !== false && $splitPosition > strlen($string) / 2) {
                    // Found another delimiter after the midpoint
                    $splitPosition++;
                    break;
                }
            }
        ...
        // Split the string at the determined position
        if ($splitPosition === false || $splitPosition >= strlen($string)) {
            // No suitable delimiter found, return the whole string
            return [$string, ''];
        } else {
            $firstPart = substr($string, 0, $splitPosition);
            $secondPart = substr($string, $splitPosition);
            return [$firstPart, $secondPart];
        }
    }
    
    
    // Usage example with your $unit->apartment_view
