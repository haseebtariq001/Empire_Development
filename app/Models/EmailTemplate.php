...
class EmailTemplate extends Model
{
    ...
    public static function sendEmailTemplate($emailTemplate, $mailTo, $obj, $user_id = null, $workspace_id = null, $attachments = null)
    {
        if (!empty($user_id)) {
        ...
        if (count(array_filter($mailTo, 'is_array')) > 0) {
            $mailTo = $mailTo[0];
        }
        ...
        if (isset($template) && !empty($template)) {
            ...
            if (!empty($content->content)) {
                ...
                // send email
                $company_setting =  company_setting('mail_from_address', $user_id, $workspace_id) ?? env('MAIL_FROM_ADDRESS');
                if (!empty($company_setting)) {
                    ...
                    if ($setconfing ==  true) {
                        try {
                            $mail = new CommonEmailTemplate($content, $content->content, $user_id, $workspace_id, $attachments);
                            if (!empty($attachments)) {
                                dispatch(new SendEmailJob($mail, $mailTo));
                            } else {
